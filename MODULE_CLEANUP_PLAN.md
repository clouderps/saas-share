...
- 🟢 **D9 — Merge `ab_approval_flow` → `ab_approval_base`.** Flow's 3 models + 2 views + 4 ACL rows folded into base. `ab_approval_integration_hr` depends list shortened (no longer references `ab_approval_flow`). None of the approval modules were installed anywhere, so no data migration. Bonus: `<tree>` → `<list>` migration on 4 view files.
- 🟢 **#95 — Approval engine actually installs.** Followup to D9: 22× `attrs="{'invisible': [...]}"` migrated to direct `invisible="..."` Python expressions across 4 view files. Two pre-existing bugs uncovered + fixed: (a) `approval_request_views.xml` referenced an undefined `action_view_related_record` external id — added the missing model method; (b) `approval_step_views.xml` referenced a non-existent `escalation_hours` field — removed (sla_hours is the actual escalation timer). End-to-end verified on DBCLOUD: install → all 7 models load → ACL gates fire → uninstall clean.
